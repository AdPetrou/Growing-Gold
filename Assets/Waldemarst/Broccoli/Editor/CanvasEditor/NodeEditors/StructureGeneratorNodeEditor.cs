...
namespace Broccoli.TreeNodeEditor
{
	...
	public class StructureGeneratorNodeEditor : BaseNodeEditor {
		...
		public override void OnInspectorGUI () {
			...
			if (selectedLevel == null) {
				...
				if (index >= 0) {
					...
					selectedToolbar = GUILayout.Toolbar (selectedToolbar, new string[] {"Structure", "Alignment", "Range"});
					EditorGUILayout.Space ();
					EditorGUI.BeginDisabledGroup (!isEnabled);
					switch (selectedToolbar) {
						case 0: // Structure
							levelChanged = StructureInspectorGUI (selectedLevel, propStructureLevel, levelChanged);
							break;
						case 1: // Alignment
							levelChanged = AlignmentInspectorGUI (selectedLevel, propStructureLevel, levelChanged);
							break;
						case 2: // Range
							levelChanged = RangeInspectorGUI (selectedLevel, propStructureLevel, levelChanged);
							break;
					}
		...
		bool StructureInspectorGUI (StructureGenerator.StructureLevel selectedLevel, SerializedProperty propStructureLevel, bool levelChanged) {
			// SPROUT GROUP
			if (selectedLevel.isSprout) {
				EditorGUI.BeginChangeCheck ();
		...
		bool AlignmentInspectorGUI (StructureGenerator.StructureLevel selectedLevel, SerializedProperty propStructureLevel, bool levelChanged) {
			EditorGUI.BeginChangeCheck ();
		...
		bool RangeInspectorGUI (StructureGenerator.StructureLevel selectedLevel, SerializedProperty propStructureLevel, bool levelChanged) {
			EditorGUI.BeginChangeCheck ();
		...
		private bool DrawNodeCanvasControls () {
			...
			if (GUILayout.Button (new GUIContent ("+ Branch Level", "Adds a branch level."))) {
			...
			// Sprout Add Button
			EditorGUI.BeginDisabledGroup (sproutStructureSelected || rootStructureSelected);
			if (GUILayout.Button (new GUIContent ("+ Sprout Level", "Adds a sprout level."))) {
				StructureGenerator.StructureLevel newLevel = 
					structureGeneratorNode.structureGeneratorElement.AddStructureLevel (
			...
			if (GUILayout.Button (new GUIContent ("+ Root Level", "Add a root level."))) {
				StructureGenerator.StructureLevel newLevel = 
					structureGeneratorNode.structureGeneratorElement.AddStructureLevel (
			...
			if (GUILayout.Button (new GUIContent ("- Remove", "Remove structure level."))) {
				StructureGenerator.StructureLevel selectedLevel =
					structureGeneratorNode.structureGeneratorElement.selectedLevel;
				curveEditor.ClearSelection ();
				if (selectedLevel != null) {
